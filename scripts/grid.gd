...
#var cardboard_preload = preload("res://scenes/cardboard.tscn")
#var cat_preload = preload("res://scenes/cat.tscn")

# 确保资源路径正确

var cat_meow_sound_path = preload("res://asserts/cat/cat-89108.wav")
...
func _ready():
	...
	if video_player:
		# 设置视频流
		video_player.stream = preload("res://asserts/cat/htkwj-7922u.ogv")
		# 自动播放视频
		video_player.play()
'''
func make_2D_array():
...
func explode_bomb(col, row):
	...
	# 触发多次 revolver_one_shot 调用的逻辑
			for i in range(15):  # 假设每次使用道具钻石时触发10次射击
				revolver_one_shot()
			await revolver_end
		"bomb":
...
func _on_revolver_shot():
	...
	"""
	#纸箱猫咪
var is_cat_box_present = false
var cat_box_prob = 0.05

func is_cleared(_x, _y):
	for i in width:
		for j in height:
			if i >= 0 and i < width and j >= 0 and j < height:
				return all_pieces[i][j].is_cleared
	return false
...
# 检查是否需要生成纸箱猫咪
func check_for_cat_box():
	if not is_cat_box_present:
		var random_x = randi() % width
		var random_y = randi() % height
		spawn_cat_box(random_x, random_y)
		is_cat_box_present = true

# 生成纸箱猫咪
func spawn_cat_box(x, y):
   # 在指定位置生成纸箱猫咪
	var cat_box_scene = preload("res://scenes/cardboard.tscn")
	var cat_box_instance = cat_box_scene.instantiate()
	cat_box_instance.position = grid_to_pixel(x, y)
	add_child(cat_box_instance)
	all_pieces[x][y] = cat_box_instance
	is_cat_box_present = true
	play_cat_meow_sound()

# 播放猫咪叫声
func play_cat_meow_sound():
	var cat_meow = preload("res://asserts/cat/cat.ogg")
	var cat_meow_instance = cat_meow.instantiate()
	add_child(cat_meow_instance)
	cat_meow_instance.play()

func check_cat_box_capture():
	for i in width:
		for j in height:
			if all_pieces[i][j] != null and all_pieces[i][j].is_cat_box():
				if is_adjacent_element_cleared(i, j):
					capture_cat(i, j)

func is_adjacent_element_cleared(x, y):
	return (is_cleared(x + 1, y) or is_cleared(x - 1, y) or
			is_cleared(x, y + 1) or is_cleared(x, y - 1))


func capture_cat(x, y):
	all_pieces[x][y].transform_to_cat()
	cat_run(x, y)
	is_cat_box_present = false

func cat_run(start_x, start_y):
	var direction = randi() % 4
	var current_x = start_x
	var current_y = start_y
	while is_within_bounds(current_x, current_y):
		all_pieces[current_x][current_y].queue_free()
		all_pieces[current_x][current_y] = null
		match direction:
			0:current_x += 1  # 向右
			1:current_x -= 1  # 向左
			2:current_y += 1  # 向上
